...
`regression-mcp-server` is an isolated Java 21 MCP Java SDK 2.0.0 STDIO server. As of Stage 15 it exposes 14 tools: 4 deterministic discovery tools (`regression_get_framework_overview`, `regression_list_modules`, `regression_list_features`, `regression_list_scenarios`), Stage 13 execution tools (`regression_start_test_run`, `regression_get_test_run`, `regression_cancel_test_run`), Stage 14 report/artifact tools (`regression_get_test_summary`, `regression_get_failure_summary`, `regression_get_failure_artifacts`, `regression_read_failure_artifact`), and Stage 15 architecture-validator tools (`regression_validate_module_boundaries`, `regression_validate_framework_conventions`, `regression_validate_architecture`). Execution v1 is intentionally limited to `regression-nextjs-commerce`, `dev`, a Boolean `headless` value, a 30–1800 second timeout, and an optional 1024-character Cucumber tag expression. Start requires `module`, `environment`, `headless`, and `timeoutSeconds`, and accepts only optional `tags`; get and cancel accept only the server-generated `runId`. The server always adds `not @wip`; a client `@cart` request becomes `(@cart) and not @wip`.

The three Stage 15 validator tools check declared reactor module source with JavaParser (no Symbol Solver) against fixed, closed-schema rule sets: `regression_validate_module_boundaries` (sibling-module and layering-boundary rules), `regression_validate_framework_conventions` (Selenium/Playwright locator discipline, no blocking waits, constructor injection, static mutable UI state), and `regression_validate_architecture` (definitions-layer discipline, package-dependency cycles, no assertions in pages/components, an advisory thin-`BasePage` check). Each accepts only optional `module`/`profile` filters and reports per-module `violations` (blocking) and, where a rule is advisory, a separate `advisoryViolations` array.
...
Stages 14 and 15 are complete; Stage 16 (final v1.0 acceptance) is upcoming and not yet started. See `STAGE_15_16_KICKOFF.md` for its original scope and `STAGE_15_PROGRESS.md` for Stage 15's final, as-built record.
